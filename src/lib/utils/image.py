...
def draw_umich_gaussian_mask(heatmap, center, radius, k=1):
  diameter = 2 * radius + 1
  gaussian = gaussian2D((diameter, diameter), sigma=diameter / 6)
...
def draw_truncate_gaussian_mask(heatmap, center, h_radius, w_radius, k=1):
  h, w = 2 * h_radius + 1, 2 * w_radius + 1
  sigma_x = w / 6
  sigma_y = h / 6
  gaussian = gaussian_2d((h, w), sigma_x=sigma_x, sigma_y=sigma_y)
...
def draw_msra_gaussian_mask(heatmap, center, sigma):
  sigma = max(1, sigma)
  tmp_size = sigma * 3
  mu_x = int(center[0] + 0.5)
  mu_y = int(center[1] + 0.5)
