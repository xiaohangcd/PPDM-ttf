...
def new_hoidet_decode(heat_obj, wh, heat_rel, offset_sub, offset_obj, corremat=None, K_obj=100, K_human=100, K_rel=100, is_sub_verb=0):
    batch, cat_obj, height, width = heat_obj.size()
    heat_obj = _nms(heat_obj)
    heat_rel = _nms(heat_rel)
    heat_human = heat_obj[:, 0, :, :].view(batch, 1, height, width)
    ...
    hoi_triplet = (torch.cat((
        sub_rel_ids.view(K_rel, 1).float(),
        obj_rel_ids.view(K_rel, 1).float(),
        clses_rel.view(K_rel, 1).float(),
        score_hoi.view(K_rel, 1)), 1)).cpu().numpy()
    ...
    wh_human = _tranpose_and_gather_feat(wh, inds_human)
    wh_obj = _tranpose_and_gather_feat(wh, inds_obj)
    wh_obj = wh_obj.view(batch, K_obj, 4)
    wh_human = wh_human.view(batch, K_human, 4)

    obj_bboxes = torch.cat([xs_obj - wh_obj[..., 0:1],
                            ys_obj - wh_obj[..., 1:2],
                            xs_obj + wh_obj[..., 2:3],
                            ys_obj + wh_obj[..., 3:4]], dim=2)

    obj_detections = torch.cat([obj_bboxes, scores_obj, clses_obj], dim=2)

    human_bboxes = torch.cat([xs_human - wh_human[..., 0:1],
                              ys_human - wh_human[..., 1:2],
                              xs_human + wh_human[..., 2:3],
                              ys_human + wh_human[..., 3:4]], dim=2)
    if is_sub_verb > 0:
        clses_human[:] = 0.0
    human_detections = torch.cat([human_bboxes, scores_human, clses_human], dim=2)
